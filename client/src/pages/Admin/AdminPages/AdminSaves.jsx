...
function AdminSaves() {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await axios.get(
        `lt-server/api/adminSaves/metadata?email=${userEmail}`
      );
    ...
    try {
      ...
      if (answer && answer.toLowerCase() === "yes") {
        const res = await axios.delete(
          `lt-server/api/admin_saves_delete/${id}`
        );
  ...
  const downloadPdf = async (filename, fileid) => {
    try {
      const response = await fetch(
        `lt-server/api/admin_report_saves?id=${fileid}`
      );
  ...
  const filterMetaData = metadata && metadata.filter((item) => {
